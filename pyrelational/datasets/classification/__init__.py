from pyrelational.datasets.classification.andrea_et_al import CreditCardDataset
from pyrelational.datasets.classification.ksenia_et_al import (
    Checkerboard2x2Dataset,
    Checkerboard4x4Dataset,
    GaussianCloudsDataset,
    StriatumDataset,
)
from pyrelational.datasets.classification.mnist import FashionMNIST
from pyrelational.datasets.classification.sklearn_datasets import (
    BreastCancerDataset,
    DigitDataset,
)
from pyrelational.datasets.classification.synthetic import (
    SynthClass1,
    SynthClass2,
    SynthClass3,
)
from pyrelational.datasets.classification.uci import UCIGlass, UCIParkinsons, UCISeeds
