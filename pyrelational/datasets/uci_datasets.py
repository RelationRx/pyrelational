    ...
    def _load_dataset(self):
        ...
        if not path.exists(self.data_dir+"UCI/" + file_name):
            urllib.request.urlretrieve(
                self.datasets[self.name], self.data_dir+"UCI/" + file_name)
        data = None

        if self.name == "housing":
            data = pd.read_csv(self.data_dir+'UCI/housing.data',
                        header=0, delimiter="\s+").values
            self.data = data[np.random.permutation(np.arange(len(data)))]

        elif self.name == "airfoil":
            data = pd.read_csv(self.data_dir+'UCI/airfoil_self_noise.dat',
                header=0, delimiter="\s+").values
            self.data = data[np.random.permutation(np.arange(len(data)))]

        elif self.name == "concrete":
            data = pd.read_excel(self.data_dir+'UCI/Concrete_Data.xls',
                               header=0).values
            self.data = data[np.random.permutation(np.arange(len(data)))]

        elif self.name == "energy":
            data = pd.read_excel(self.data_dir+'UCI/ENB2012_data.xlsx',
                                 header=0).values
            self.data = data[np.random.permutation(np.arange(len(data)))]
            ...
            data = pd.read_csv(self.data_dir + 'UCI/winequality-red.csv',
                               header=1, delimiter=';').values
            self.data = data[np.random.permutation(np.arange(len(data)))]

        elif self.name == "yacht":
            data = pd.read_csv(self.data_dir + 'UCI/yacht_hydrodynamics.data',
